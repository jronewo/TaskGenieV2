...
const LoginScreen = ({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setApiError(messageFromError(err, "Unable to sign in. Please try again."));
    } finally {
      setLoading(false);
    }
  };
...
// Forgot Password — backend has no forgot/reset-password endpoint yet (PROD-0101, not built).
// We surface that honestly instead of faking an OTP/reset flow that would silently do nothing server-side.
const ForgotPasswordScreen = ({ onBack }: { onBack: () => void }) => (
  <motion.div {...SLIDE}>
    ...
    <div className="mb-4">
      <div className="w-12 h-12 bg-amber-50 border border-amber-100 rounded-lg flex items-center justify-center mb-4">
        <Lock size={22} className="text-amber-700" />
      </div>
      <h2 className="text-2xl font-bold text-gray-900 mb-1">Password reset unavailable</h2>
      <p className="text-sm text-gray-500 leading-relaxed">
        Self-service password reset isn't available in this environment yet. Please contact your
        workspace administrator to reset your password.
      </p>
    </div>
    <AuthButton variant="ghost" onClick={onBack}>Back to Login</AuthButton>
  </motion.div>
);
