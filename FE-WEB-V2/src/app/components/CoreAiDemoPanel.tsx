...
export const CoreAiDemoPanel = () => {
  ...
  const analyzeRisk = () => run("risk", async () => {
    const assessment = await coreAiApi.analyzeRisk(taskId);
    setRisk(assessment);
    setHistory(await coreAiApi.getRiskHistory(taskId));
    toast.success(`Risk ${assessment.riskLevel}: ${assessment.totalScore}/100`);
  });

  const recommend = () => run("recommend", async () => {
    const response = await coreAiApi.recommend(taskId, projectId);
    setRecommendations(response);
    toast.success(`${response.suggestions.length} candidates ranked.`);
  });

  const refreshEvidence = () => run("evidence", async () => {
    setEvidence(await coreAiApi.getEvidence(taskId));
  });

  const addEvidence = () => run("add-evidence", async () => {
    await coreAiApi.addUrlEvidence(taskId, evidenceUrl, "Demo evidence");
    setEvidenceUrl("");
    setEvidence(await coreAiApi.getEvidence(taskId));
  ...
  const decide = (candidateId: number, accepted: boolean) => run(`decision-${candidateId}`, async () => {
    if (accepted) await coreAiApi.acceptRecommendation(taskId, candidateId);
    else await coreAiApi.rejectRecommendation(taskId, candidateId);
