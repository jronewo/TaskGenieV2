...
public class GetAllActivitiesQueryHandler(
    ...
    : IRequestHandler<GetAllActivitiesQuery, List<ActivityLogDto>>
{
    public async Task<List<ActivityLogDto>> Handle(GetAllActivitiesQuery request, CancellationToken ct)
    {
        var logs = await activityLogRepository.GetAllAsync(request.Limit, ct);
