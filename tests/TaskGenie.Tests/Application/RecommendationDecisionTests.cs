...
public sealed class RecommendationDecisionTests
{
    ...
    public async Task Accept_AlreadyAssigned_DoesNotToggleAssignmentOff()
    {
        ...
        var handler = new AcceptAssignmentRecommendationCommandHandler(taskRepo.Object, recommendationRepo.Object, Mock.Of<IMediator>());

        var result = await handler.Handle(new AcceptAssignmentRecommendationCommand(5, 10, 1), CancellationToken.None);
    ...
    public async Task Accept_UserOutsideLatestRun_IsRejectedBeforeChangingAssignment()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            handler.Handle(new AcceptAssignmentRecommendationCommand(5, 999, 1), CancellationToken.None));
